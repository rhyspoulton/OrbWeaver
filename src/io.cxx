...
vector<HaloData> ReadSnapshotData(int snap, Group snapgroup, Options &opt, vector<SnapData> &snapdata, HDFCatalogNames hdfnames){
	...
	for(int n=0;n<snapdata[snap].numhalos;n+=ichunk){
		...
		for(int nn=0;nn<ichunk;nn++) Halo[count++].origrootprogenitor = longbuff[nn];


		// original root descendant
		ifield++;
		count=offset;
		rank = 1;
		dim[0] = ichunk;
		...
		for(int nn=0;nn<ichunk;nn++) Halo[count++].origrootdescendant = longbuff[nn];
...
void WriteOrbitData(Options &opt, vector<OrbitData> &orbitdata){
	...
	try{
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) ullongbuf[j] = orbitdata[j].halorootprogenID;
		dataset.write(ullongbuf,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) ullongbuf[j] = orbitdata[j].halorootdescenID;
		dataset.write(ullongbuf,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) llongbuf[j] = orbitdata[j].orbitedhaloorigrootprogenID;
		dataset.write(llongbuf,hdfdatasetnames.datasettypes[idataset]);
		idataset++;

		/* orbitedhaloorigrootdescen */
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) llongbuf[j] = orbitdata[j].orbitedhaloorigrootdescenID;
		dataset.write(llongbuf,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].totnumorbits;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].closestapproachscalefactor;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].orbitalenergy_inst;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].orbitalenergy_ave;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].vcirc;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].jcirc;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].minrmax;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].minrscale;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].r;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].vrel;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].accrel;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].minacc;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].maxacc;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].lrel_inst;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
		...
		//Write out the dataset
		for(unsigned int j=0; j<numentries;j++) floatbuff[j] = orbitdata[j].lrel_ave;
		dataset.write(floatbuff,hdfdatasetnames.datasettypes[idataset]);
		idataset++;
